...
vector<Image*> ImageStorage::itemImage;
...
{	
	for(int i = 0; i < texCoords.size(); ++i){
		if(texCoords[i][4] == br[X] && texCoords[i][5] == br[Y]){
			if(texCoords[i][2] == tl[X] && texCoords[i][3] == tl[Y])
				return texCoords[i];
		}
	}
...
void ImageStorage::addItemImage(Image* img)
{
	itemImage.push_back(img);
}

Image* ImageStorage::getItemImage(GLshort i)
{
	return itemImage[i];
}
