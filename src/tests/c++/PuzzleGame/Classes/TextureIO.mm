...
Image* loadPNG(const char* filename, const Size2d<GLshort>& rs){
	...
	[image release];
	[texData release];
	return new Image(texName, rs, Size2d<GLshort>(width, height));
}
...
vector<Image*> loadPNG(const char* filename){	
	...
	NSString* txcpath = [[NSBundle mainBundle] pathForResource:[NSString stringWithCString:filename] ofType:@"txc"];
	...
	// Check if png exists
	printf("Loading %s.png...\n", filename);
	NSString* imgpath = [[NSBundle mainBundle] pathForResource:[NSString stringWithCString:filename] ofType:@"png"];
