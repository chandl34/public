...
void Heap::shiftUp(Node* node){
	while(node != head && compare(node->getValue(), node->getParent()->getValue())){
...
void Heap::shiftDown(Node* node){
	if(node->getLeft() != NULL){
		if(node->getRight() != NULL){
			if(!compare(node->getLeft()->getValue(), node->getRight()->getValue())){
				if(compare(node->getRight()->getValue(), node->getValue())){
		...
		if(compare(node->getLeft()->getValue(), node->getValue())){
...
bool Heap::compare(GLfloat v1, GLfloat v2) const{
	if(minHeap)
		return v1 < v2;
	else
		return v1 > v2;
